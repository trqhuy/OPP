...
void Admin::themKhachHangTietKiem(TaiKhoanTietKiem& TaiKhoanTietKiem) {
	...
	// Tạo file lưu thông tin khách hàng
	string idBank = soThe.substr(0, 3); // lấy 3 ký tự đầu tiên của số thẻ
	ofstream outputFile("Bank_" + idBank + "_Tiet_Kiem_information.txt", ios::app);
	if (outputFile.is_open()) {
...
void Admin::themKhachHangVayVon(TaiKhoanVayVon& TaiKhoanVayVon) {
	...
	// Tạo file lưu thông tin khách hàng
	string idBank = soThe.substr(0, 3); // lấy 3 ký tự đầu tiên của số thẻ
	ofstream outputFile("Bank_" + idBank + "_Vay_Von_information.txt", ios::app);
	if (outputFile.is_open()) {
...
void Admin::xoaTaiKhoanTietKiem(string soThe) {
	string idBank = soThe.substr(0, 3);
	ifstream inputFile("Bank_" + idBank + "_Tiet_Kiem_information.txt");
	ofstream tempFile(idBank + "_temp.txt");
	bool found = false;
	...
	if (inputFile.is_open() && tempFile.is_open()) {
		...
		inputFile.close();
		tempFile.close();
		remove(("Bank_" + idBank + "_Tiet_Kiem_information.txt").c_str());
		rename((idBank + "_temp.txt").c_str(), ("Bank_" + idBank + "_Tiet_Kiem_information.txt").c_str());
	} else {
		cout << "!!!!!!!!Khong the mo file de xoa!!!!!!!!" << endl;
	}
...
void Admin::xoaTaiKhoanVayVon(string soThe) {
	string idBank = soThe.substr(0, 3);
	ifstream inputFile("Bank_" + idBank + "_Vay_Von_information.txt");
	ofstream tempFile(idBank + "_temp.txt");
	bool found = false;
	...
	if (inputFile.is_open() && tempFile.is_open()) {
		...
		inputFile.close();
		tempFile.close();
		remove(("Bank_" + idBank + "_Vay_Von_information.txt").c_str());
		rename((idBank + "_temp.txt").c_str(), ("Bank_" + idBank + "_Vay_Von_information.txt").c_str());
	} else {
		cout << "!!!!!!!!Khong the mo file de xoa!!!!!!!!" << endl;
	}
