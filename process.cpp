...
public:
    ...
    void ghiLichSu(string action, float amount);
    void ghiLichSuNganHang(string action, float amount);
...
void ATM::ghiLichSu(string action, float amount) {
    ...
    if (logFile.is_open()) {
        time_t now = time(0);
        char* dt = ctime(&now);
        logFile << dt << ": " << action << " - " << amount << " VND" << endl;
        logFile.close();
    } else {
...
void ATM::ghiLichSuNganHang(string action, float amount) {
    string idBank = Cust.soThe.substr(0, 3);
    ...
    if (bankLogFile.is_open()) {
        time_t now = time(0);
        char* dt = ctime(&now);
        bankLogFile << dt << ": " << action << " - " << amount << " VND, Tai khoan: " << Cust.soThe << endl;
        bankLogFile.close();
    } else {
...
void ATM::rutTien(float soTien) {
    ...
    ghiLichSu("Rut tien", soTien);
    ghiLichSuNganHang("Rut tien", soTien);
    ghiThongTinKhachHang();
    cout << "Rut tien thanh cong: " << soTien << " VND" << endl;
    cout << "So du sau khi rut: " << Cust.soDu << " VND" << endl;
}

void ATM::napTien(float soTien) {
    Cust.setSoDu(Cust.soDu + soTien);
    ghiLichSu("Nap tien", soTien);
    ghiLichSuNganHang("Nap tien", soTien);
    ghiThongTinKhachHang();
    cout << "Nap tien thanh cong: " << soTien << " VND" << endl;
    cout << "So du hien tai: " << Cust.soDu << " VND" << endl;
}
...
public:
    ...
    void themTaiKhoan(Customer& customer) {
        themKhachHang(customer);
    }
