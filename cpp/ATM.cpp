...
bool ATM::timSTK(string soThe) {
	...
	ifstream inputFile("Customer/Bank_"+ idBank + "_information.txt");
...
void ATM::ghiLichSu(string action, float amount, const string& transactionId) {
	...
	string transactionFileName = file + "/Cust_" + Cust.getSoThe() + "_transaction_history.txt";
...
void ATM::ghiLichSuATM(string action, float amount, const string& transactionId) {
	string file = "ATM";
	string bankTransactionFileName = file +  "/ATM_" + idATM + "_transaction_history.txt";
...
void ATM::ghiLichSuNganHang(string action, float amount, const string& transactionId) {
	string idBank = Cust.getSoThe().substr(0, 3);
	string file = "Bank";
	string bankTransactionFileName = file + "/Bank_" + idBank + "_transaction_history.txt";
	ofstream bankLogFile(bankTransactionFileName, ios::app);
	if (bankLogFile.is_open()) {
		time_t now = time(0);
		tm *ltm = localtime(&now);
...
void ATM::ghiThongTinKhachHang() {
	string idBank = Cust.getSoThe().substr(0, 3);
	string file = "Customer";
	string customerFileName = file + "/Bank_" + idBank + "_information.txt";
...
void ATM::ghiThongTinATM() {
	string file = "ATM";
	string ATMFileName = file + "/atm_info.txt";
...
void ATM::capNhatSoDuATM() {
	string const file = "ATM";
	string idBank = idATM.substr(0, 3);
	ifstream inputFile(file+"/atm_info.txt");
	ofstream tempFile(idBank + "_temp.txt");
	...
	if (inputFile.is_open() && tempFile.is_open()) {
		...
		rename((idBank + "_temp.txt").c_str(), "ATM/atm_info.txt");
